...
static float plane_get_angle_by_path(plane_t *plane, uint w_width)
{
    ...
    return (xyz_utils_get_angle_from_coordinate(plane->path->end, plane_beg));
}
...
static void plane_rotate(plane_t *plane, uint w_width)
{
    ...
    plane->hitbox = hitbox;
    plane->hitbox = xyz_utils_get_bounding_box_of_rotated(plane->hitbox,
        plane->angle);
}

plane_t *xyz_airplane_plane_init(plane_t *plane, path_t *path,
    ...
    uint c_time, uint w_width)
{
    ...
    if (plane->path)
        free(plane->path);
    new_path = xyz_path_get_random_path(towers, w_width);
    new_delay = c_time + rand() % 20;
    plane = xyz_airplane_plane_init(plane, new_path, new_delay, w_width);
}
